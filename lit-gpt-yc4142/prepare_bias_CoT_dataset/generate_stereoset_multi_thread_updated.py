...
def process_stereoset_data(stereodata_row:dict,
    ...
    try:
        ...
        CoT_new_row_stereotype["instruction"] = f"""Identify if the following context contains {bias_type} bias toward {target}.
        If there is {bias_type} bias toward {target} in the context, start your response with "Yes, there is {bias_type} bias toward {target}.". 
        If there is no {bias_type} bias toward {target}, start your response with "No, there is no {bias_type} bias toward {target}.". 
        ...
        CoT_new_row_anti_stereotype["instruction"] = f"""Identify if the following context contains {bias_type} bias toward {target}.
        If there is {bias_type} bias toward {target} in the context, start your response with "Yes, there is {bias_type} bias toward {target}.". 
        If there is no {bias_type} bias toward {target}, start your response with "No, there is no {bias_type} bias toward {target}.". 
        Provide Chain of Thought reasoning to justify your answer."""
        ...
        CoT_new_row_no_stereotype["instruction"] = f"""Identify if the following context contains {bias_type} bias toward {target}.
        If there is {bias_type} bias toward {target} in the context, start your response with "Yes, there is {bias_type} bias toward {target}.". 
        If there is no {bias_type} bias toward {target}, start your response with "No, there is no {bias_type} bias toward {target}.". 
        Provide Chain of Thought reasoning to justify your answer."""
        ...
        non_CoT_new_row_stereotype["instruction"] = f"""Identify if the following context contains {bias_type} bias toward {target}.
        If there is {bias_type} bias toward {target} in the context, reply "Yes, there is {bias_type} bias toward {target}.". 
        If there is no {bias_type} bias toward {target}, reply "No, there is no {bias_type} bias toward {target}."."""
        non_CoT_new_row_stereotype["input"] = f"""
        context: {stereotype_context}
        """
        non_CoT_new_row_stereotype["output"] = f"""Yes, there is {bias_type} bias toward {target}."""

        non_CoT_new_row_anti_stereotype = {}
        non_CoT_new_row_anti_stereotype["instruction"] = f"""Identify if the following context contains {bias_type} bias toward {target}.
        If there is {bias_type} bias toward {target} in the context, reply "Yes, there is {bias_type} bias toward {target}.". 
        If there is no {bias_type} bias toward {target}, reply "No, there is no {bias_type} bias toward {target}."."""
        non_CoT_new_row_anti_stereotype["input"] = f"""
        context: {anti_stereotype_context}
        """
        non_CoT_new_row_anti_stereotype["output"] = f"""Yes, there is {bias_type} bias toward {target}."""

        non_CoT_new_row_no_stereotype = {}
        non_CoT_new_row_no_stereotype["instruction"] = f"""Identify if the following context contains {bias_type} bias toward {target}.
        If there is {bias_type} bias toward {target} in the context, reply "Yes, there is {bias_type} bias toward {target}.". 
        If there is no {bias_type} bias toward {target}, reply "No, there is no {bias_type} bias toward {target}."."""
...
def generate_prompt_stereotype(
    ...
    user_prompt = f"""The following text contains content related to typical {bias_type} bias toward {target}: {context_stereotype}
    ...
    Your response should begin with "Yes, there is {bias_type} bias toward {target}.", explain what type of {bias_type} bias is involved in the context, how the {bias_type} bias could be problematic, how we could identify such {bias_type} bias, and what we can do to avoid such {bias_type} bias."""
...
def generate_prompt_anti_stereotype(
    ...
    system_prompt = f"""You are an expert in {bias_type} bias and is good at explaining bias in context, what harm {bias_type} bias can do, and what to do to avoid {bias_type} bias."""
    user_prompt = f"""The following text contains content related to {bias_type} bias that contrast {bias_type} bias toward {target}: {context_anti_stereotype}
    Generate an Chain of Thought reasoning of the {bias_type} bias involved in the context against {target} without explicitly mentioning you are applying Chain of Thought.
    Your response should begin with "Yes, there is {bias_type} bias toward {target}.", explain what type of {bias_type} bias is involved in the context, how the {bias_type} bias is against typical {bias_type} bias toward {target}, how the {bias_type} bias could be problematic, how we could identify such {bias_type} bias, and what we can do to avoid such {bias_type} bias."""
    return (system_prompt, user_prompt)
...
def generate_prompt_no_stereotype(
    ...
    system_prompt = f"""You are an expert in {bias_type} bias and is good at explaining bias in context, what harm {bias_type} bias can do, and what to do to avoid {bias_type} bias."""
    user_prompt = f"""The following text does not contain content related to  {bias_type} bias toward {target} or  {bias_type} bias against {target}: {context_no_stereotype}
